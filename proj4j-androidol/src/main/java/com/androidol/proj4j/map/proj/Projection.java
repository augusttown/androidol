...
public class Projection implements Cloneable {
	...
	protected GeometryFactory jtsGeometryFactory = new GeometryFactory();
  ...
  private double totalFalseNorthing = 0;
	...
	public Point transform( Point src, Point dst ) {
		double x = src.getX()*DTR;
		if ( projectionLongitude != 0 )
			x = MapMath.normalizeLongitude( x-projectionLongitude );
		project(x, src.getY()*DTR, dst);
		dst. = totalScale * dst.x + totalFalseEasting;
	...
	*/
	...
	/**
	 * Project a lat/long point (in degrees), producing a result in meters
	 */
	public Point transform(Point src) {
		double x = src.getX()*DTR;
		if ( projectionLongitude != 0 )
			x = MapMath.normalizeLongitude( x-projectionLongitude );
		Point dst = project(x, src.getY()*DTR);		
		dst = this.jtsGeometryFactory.createPoint(
			new Coordinate(
				totalScale * dst.getX() + totalFalseEasting,
				totalScale * dst.getY() + totalFalseNorthing
			)
		);		
		return dst;
	}
	 ...
	 */
	/*
	public Point transformRadians( Point src, Point dst ) {
		double x = src.getX();
		if ( projectionLongitude != 0 )
			x = MapMath.normalizeLongitude( x-projectionLongitude );
		project(x, src.y, dst);
	...
	*/
	 ...
	 */
	/*
	public Point project(double x, double y, Point dst) {
		dst.x = x;
		dst.y = y;
		return dst;
	}
	...
	*/
	...
	public Point inverseTransform(Point src) {
		double x = (src.getX() - totalFalseEasting) / totalScale;
		double y = (src.getY() - totalFalseNorthing) / totalScale;
		Point dst = projectInverse(x, y);
		...
		if (dst.getX() < -Math.PI)
			...
			dstX = Math.PI;
		if (projectionLongitude != 0)
			dstX = MapMath.normalizeLongitude(dstX + projectionLongitude);
		dstX *= RTD;
		dstY *= RTD;
		return this.jtsGeometryFactory.createPoint(new Coordinate(dstX, dstY));
	}
	
	/**
	 * Inverse-project a point (in metres), producing a lat/long result in degrees
	 */
	public Point inverseTransform(double srcX, double srcY) {
	 ...
	 */
	/*
	public Point inverseTransformRadians(Point src, Point dst) {
		double x = (src.getX() - totalFalseEasting) / totalScale;
		double y = (src.getY() - totalFalseNorthing) / totalScale;
		projectInverse(x, y, dst);
		if (dst.getX() < -Math.PI)
			...
			dst.getX() = Math.PI;
		if (projectionLongitude != 0)
			dst.getX() = MapMath.normalizeLongitude(dst.getX()+projectionLongitude);
		return dst;
	}
	*/
