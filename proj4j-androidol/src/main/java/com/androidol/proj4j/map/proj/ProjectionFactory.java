...
//pm
	...
	static Hashtable<String, Class<? extends Projection>> registry;

	static void register( String name, Class<? extends Projection> cls, String description ) {
	...
	static Projection getNamedPROJ4Projection( String name ) {
		if ( registry == null )
			initialize();
		Class<? extends Projection> cls = (Class<? extends Projection>)registry.get( name );
		if ( cls != null ) {
			try {
	...
	static void initialize() {
		registry = new Hashtable<String, Class<? extends Projection>>();
		/*
