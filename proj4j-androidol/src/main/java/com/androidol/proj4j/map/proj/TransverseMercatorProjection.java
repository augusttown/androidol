...
public class TransverseMercatorProjection extends CylindricalProjection {
	...
	*/
	...
	public Point project(double lplam, double lpphi) {
		double outX;
		double outY;
		if (spherical) {
			double cosphi = Math.cos(lpphi);
			...
			outX = ml0 * scaleFactor * Math.log((1. + b) / (1. - b));
			double ty = cosphi * Math.cos(lplam) / Math.sqrt(1. - b * b);
			ty = MapMath.acos(ty);
			if (lpphi < 0.0)
				ty = -ty;
			outY = esp * (ty - projectionLatitude);
		} else {
			double al, als, n, t;
			double sinphi = Math.sin(lpphi);
			double cosphi = Math.cos(lpphi);
			...
			al /= Math.sqrt(1. - es * sinphi * sinphi);
			n = esp * cosphi * cosphi;
			outX = scaleFactor * al * (FC1 +
				FC3 * als * (1. - t + n +
				FC5 * als * (5. + t * (t - 18.) + n * (14. - 58. * t)
				+ FC7 * als * (61. + t * ( t * (179. - t) - 479. ) )
			)));
			outY = scaleFactor * (MapMath.mlfn(lpphi, sinphi, cosphi, en) - ml0 +
				sinphi * al * lplam * FC2 * ( 1. +
				FC4 * als * (5. - t + n * (9. + 4. * n) +
				FC6 * als * (61. + t * (t - 58.) + n * (270. - 330 * t)
				+ FC8 * als * (1385. + t * ( t * (543. - t) - 3111.) )
	...
	public Point projectInverse(double x, double y) {
		...
		if (spherical) {
			...
			double g = .5 * (h - 1. / h);
			h = Math.cos(projectionLatitude + y / scaleFactor);
			outY = MapMath.asin(Math.sqrt((1. - h*h) / (1. + g*g)));
			if (y < 0)
				outY = -outY;
			outX = Math.atan2(g, h);
		} else {
			...
			outY = MapMath.inv_mlfn(ml0 + y/scaleFactor, es, en);
			if (Math.abs(y) >= MapMath.HALFPI) {
